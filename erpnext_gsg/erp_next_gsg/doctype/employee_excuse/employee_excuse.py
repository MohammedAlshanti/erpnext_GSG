...
class EmployeeExcuse(Document):
	def validate(self):
		...
		excuse_hours_dp = frappe.db.sql(f""" select excuse_hours_alowed from `tabDepartment`
		where name = '{self.department}' """)[0][0]
		applied_in_the_month = frappe.db.sql(f""" select hours from `tabEmployee Excuse`
		where employee = '{self.employee}'
		and excuse_date between '{get_first_day(today())}' and '{get_last_day(today())}'""")[0][0]
